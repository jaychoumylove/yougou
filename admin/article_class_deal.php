<?
	...
	if($act=="update"){//执行修改操作
		...
		$sql="update `yg_news_class` set `classname`='".$classname."', `description`='".$description."' where Id=".$myId;
		//exit($sql);
		...
		$sql="insert into yg_news_class (classname,description,addtime) values('".$classname."','".$description."','".$date."')";
		//exit($sql);
		...
		$Id=$_GET["Id"];
		$sql="delete from `yg_news_class` where `Id`=".$Id;
		$result=mysql_query($sql);
		...
		$sql="delete from `yg_news_class` where Id in (".$Id_str.")";
		$result=mysql_query($sql);
		$affet_rows=mysql_affected_rows();//记录受影响行		
		if($result && $affet_rows){
			echo "<script>window.location.href='article_class.php';alert('删除成功！');</script>";
